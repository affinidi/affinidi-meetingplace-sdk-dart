...
The capability matrix below covers per-chat actions that differ between transports. These are the features the `ChatFeature` enum gates in the UI. Identity and credential features that ride on top of chat (credential exchange, ZKP liveness, R-Card sharing) are not transport-gated; see [Identity and credential features](#identity-and-credential-features).
...
| ZKP liveness | Zero-knowledge liveness concierge exchange | Yes | Yes | Transport-agnostic. The SDK exposes the liveness ZKP concierge mappers and derivers, which behave the same on either transport. |
...
These features stay out of the `ChatFeature` enum on purpose. The enum hides or disables per-chat actions that differ between transports, and these do not. Credential exchange varies by chat type (individual versus group), not by transport, so that distinction belongs to the per-chat-type design, not the transport capability set.
