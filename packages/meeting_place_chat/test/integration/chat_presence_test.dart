...
void main() {
  ...
  test('start chat presence updates after restarting chat session', () async {
    ...
    expect(
      bobMessages.any((IncomingMessage m) {
        if (m is! DidCommIncomingMessage) return false;
        if (m.payload.isOfType(type) == false) {
          return false;
        }

        final message = ChatPresence.fromPlainTextMessage(m.payload);
