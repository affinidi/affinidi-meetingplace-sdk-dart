...
Decentralised Identifiers (DIDs) and DIDComm v2.1.
...
provide environment variables for `CONTROL_PLANE_DID` and `MEDIATOR_DID`.
The examples use the [dotenv](https://pub.dev/packages/dotenv) package to
load these values from a local `.env` file for convenience.
...
1. **Create your local environment file**
   ...
   Edit `.env` and update the values for `CONTROL_PLANE_DID` and `MEDIATOR_DID`
