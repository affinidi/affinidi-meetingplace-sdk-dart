...
class GroupService {
  ...
  createGroup({
    ...
    try {
      ...
      final roomId = await _channelTransport.setupChannel(
        channel: channel,
        didManager: ownerDid,
        participantDids: [
      ...
      if (ownerIdentity.agentDid != null) {
  ...
  Future<AcceptGroupOfferResult> acceptGroupOffer({
    ...
    try {
      ...
      _logger.info(
        'Successfully accepted group offer: '
        '${acceptedConnectionOffer.offerLink}',
        name: methodName,
      );
  ...
  Future<void> sendAcceptInvitationGroupToMediator({
    ...
    _logger.info(
      'Started sending accept invitation to mediator: '
  ...
  Future<Channel> approveMembershipRequest({required Channel channel}) async {
    ...
    _logger.info(
      'Group member DIDs: ${group.members.length} members: '
      '[${group.members.map((m) => m.did.topAndTail()).join(', ')}], '
      'approving member DID: ${memberDid.topAndTail()}',
      name: methodName,
    );
  ...
  Future<void> leaveGroup(Channel channel) async {
    ...
    if (channel.notificationToken != null) {
      _logger.info(
        'Deregistering notification token for channel with offer link: '
        '${channel.offerLink}',
        name: methodName,
      );
      await _controlPlaneSDK.execute(
    ...
    _logger.info(
      'Successfully left group for channel with offer link: '
      '${channel.offerLink}',
      name: methodName,
    );
  }
  ...
  Future<void> _leaveGroupAsAdmin(
    ...
    await _channelTransport.sendEvent(
      channel: channel,
      type: 'com.affinidi.chat.group-deletion',
  ...
  Future<void> _leaveGroupAsMember({
    ...
    try {
      await _channelTransport.leaveChannel(
        channel: channel,
