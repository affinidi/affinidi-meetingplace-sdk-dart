...
void main() {
  ...
  group('createPermanentIdentity', () {
    ...
    test(
      '''authenticates on channel transport and notifies agent when transport is matrix''',
      () async {
        when(
        ...
        verify(
          () => mockMeetingPlaceTransport.authenticate(mockDidManager),
        ).called(1);
        ...
        expect(message.body!['transport'], equals('matrix'));
      },
    );
    ...
    test('sends request with correct body to agent DID', () async {
      ...
      expect(message.body!['transport'], equals('didcomm'));
