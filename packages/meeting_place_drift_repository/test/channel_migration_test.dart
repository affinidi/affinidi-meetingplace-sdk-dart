...
void main() {
  ...
  group('v4 → v5 schema migration', () {
    ...
    test('adds matrix user ID columns with NULL as default', () async {
      ...
      final rows = await db.customSelect(
        'SELECT matrix_user_id, other_party_matrix_user_id '
        'FROM channels WHERE id = ?',
        variables: [const Variable('ch-4')],
      ).get();

      expect(rows.single.read<String?>('matrix_user_id'), isNull);
      expect(
        rows.single.read<String?>('other_party_matrix_user_id'),
        isNull,
      );
