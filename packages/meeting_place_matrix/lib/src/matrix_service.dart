...
import 'package:meeting_place_core/meeting_place_core.dart'
    show Channel, ChannelType, MeetingPlaceCoreSDKLogger;
...
class MatrixService {
  ...
  ///
  /// When [since] is provided, uses `/context/{since}` to obtain
  ...
  /// strictly newer than [since] are fetched from the homeserver.
  ///
  /// When [since] is null, calls `requestHistory(direction: f)` from
  ...
  Future<List<MatrixRoomEvent>> fetchRoomHistory(
    ...
    if (since != null) {
      final context = await client.getEventContext(roomId, since, limit: 0);
    ...
    final events = timeline.events
        .takeWhile((e) => since == null || e.eventId != since)
  ...
  /// Returns the authenticated [matrix.Client] for [didManager], establishing
  /// a session if one has not been set up yet.
  ///
  /// Consumer packages that implement Matrix-specific features (e.g. VoIP,
  /// OpenID token exchange) use this to obtain a client without holding a
  /// direct reference to the session manager.
  Future<matrix.Client> authenticatedClient(DidManager didManager) =>
      _ensureSession(didManager);
