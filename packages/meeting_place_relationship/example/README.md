# meeting_place_relationship examples

Check the sample code to learn how to use the Affinidi Meeting Place - Relationship SDK package to exchange R-Cards and Verifiable Relationship Credentials (VRCs) with peers over DIDComm v2.

| File path                 | What it demonstrates                                                                                                         |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `relationship/alice.dart` | Publishes a connection offer, approves Bob's request, sends an R-Card, initiates a VRC exchange, and reciprocates Bob's VRC. |
| `relationship/bob.dart`   | Finds and accepts Alice's offer, sends an R-Card, and responds to Alice's VRC request.                                       |
...
Execute the example Dart scripts from the `packages/meeting_place_relationship/example` folder. The wired examples (`relationship/`) require environment variables for `CONTROL_PLANE_DID` and `MEDIATOR_DID`. The examples use the [dotenv](https://pub.dev/packages/dotenv) package to load these values from a local `.env` file for convenience.
...
2. **Run the example scripts**
   ...
   ```bash
   # Wired relationship example (run in two separate terminals simultaneously)
   dart run relationship/alice.dart
   dart run relationship/bob.dart
   ```

   Alice writes a mnemonic to `.example-output/relationship-storage.txt` and waits. Start Bob once Alice is waiting.
